...
import styles from "./ThrusterInverts.css";
...
export default class ThrusterScales extends Component {
    ...
    rendLeftScales() {
        return [0, 1, 2, 3].map(function(val, index) {
                    return (
                        <SliderControl min='0' max='100' key={'thrust'+val} indx={val} invert={that.state.scales[val].invert} rend={that.rendData.bind(that)} name={"Thruster "+(val+1)} />
                    );
                });
    }
